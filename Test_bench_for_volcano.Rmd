...
```{r}
prepare_volcano_of_given_property = function(data, property.to.plot, property.column,
                                             property.thr, logfc.thr, main, 
                                             numlab, point.lab, sym.col){
  
  data[,property.to.plot] <- data[[property.column]]
  ...
  data[,name.of.legend] <- "not signif."
  data[,name.of.legend][data[,property.to.plot] < property.thr] <- paste(legend.treshold.tag, fdr.thr, sep="<")
  
  #Adding another bivalue column for magnitude of foldchange
  data$FC <- paste0("less than ", 2^logfc.thr, "-fold")
  data$FC[(data$logFC < -1*logfc.thr | data$logFC > logfc.thr)] <- paste0("more than ", 2^logfc.thr, "-fold")
  
  #Filtering data to color points
  filtdat <- data[ data[,property.to.plot] < property.thr, ]
  print(dim(filtdat))
  cat("    ", nrow(filtdat), "point(s) labelled...\n")
  if (nrow(filtdat) > numlab) {
    cat("     Restricting to", numlab, "...\n")
  ...
  volcano.plot <- ggplot(data, aes(x = data$logFC, y = -log10(data[,property.to.plot]), 
                                   color = data[,name.of.legend], fill=data[,name.of.legend]))
  ...
  volcano.plot <- volcano.plot + geom_vline(aes(xintercept=-logfc.thr), colour="red", linetype="dashed")
    ...
    volcano.plot <- volcano.plot + geom_text(data=data, aes(x=floor(min(data$logFC)), y=-log10(property.thr)), label = line_label_text, nudge_x=-1.5, nudge_y=max(-log10(data[,property.to.plot]))/60, size=3, color="red")
...
diff_expr_volcano_plot <-
  function(d3, id, sym.col="gene_symbol", main=NULL, p.thr=0.05, fdr.thr=0.05, logfc.thr=1, numlab=15, point.lab=TRUE)
  {
   ...
   g.l <- list()
   g.l[["FDR"]] = prepare_volcano_of_given_property(data = cont.dat, 
   ...
   # g.l[["Pval"]] = prepare_volcano_of_given_property(data = cont.dat, 
   #                                                   property.to.plot = 'p', 

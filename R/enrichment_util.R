...
runEnrichmentAnalyses <- function(diffr_wrapper.output, analysis.name="", 
                                  ...
                                  clusterProfiler.params = list(do.similarity.filtering=F,max.gene.set.size = 1000),
    ...
    for (method in enrichment.methods) { 
     ...
     if(method == "clusterProfilerGO"){
       ...
       enrichment_out.l$clusterProfiler_ORA[[contrast]] = run_clusterProfiler_GO(input_genes = input.genes,
                                                                                 ...
                                                                                 OrgDb = org.db,
                                                                                 pvalueCutoff = p.thr,
                                                                                 max_set_size = clusterProfiler.params$max.gene.set.size,
                                                                                 pAdjustMethod = "BH",
                                                                                 similarity_filtering = clusterProfiler.params$do.similarity.filtering)
       ...
       print(clusterProfiler.params$max.gene.set.size)
      ...
      if(method == "DAVID") {
        ...
        if(david.params$email.address != "") {
          ...
          enrichment_out.l$DAVID[[contrast]] <- doDavidEnrichmentAnalysis(background.ensembl.ids = background.genes,
                                   ...
                                   pval.thr = p.thr, 
                                   max.gene.set.size = david.params$max.gene.set.size, save.result.table = TRUE, out.dir=out.dir,analysis.name,

...
prepare_volcano_of_given_property = function(data.df, property.to.plot, property.column,
  ...
  volcano.plot <- volcano.plot + scale_x_continuous(breaks = scales::pretty_breaks(n=6)) + scale_y_continuous(breaks = scales::pretty_breaks(n=6))
  ...
  volcano.plot <- volcano.plot + ggtitle(main, subtitle = subtitle.name) + 
    ...
    volcano.plot <- volcano.plot + geom_text(data=data.df, aes(x=floor(min(data.df$logFC)), y=-log10(property.thr)), label = line_label_text, nudge_x=0.5, nudge_y=max(-log10(data.df[,property.to.plot]))/70, size=4, color="red")

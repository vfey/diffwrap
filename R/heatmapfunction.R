...
#' @param color.blind.pal string determining the RColorBrewer color blind palette (default = "PuOr");
#' other option can be visualized with the following command: brewer.pal.info[brewer.pal.info$colorblind,]
#' @param sign.val \code{character}. Type of significance value that was used to filter the values. One of 'FDR' and 'P-value'.
#' @details
...
diffr_pheatmap = function(expr.mat, clinical.mat,
                          ...
                          sign.val = c("FDR", "P-value")) {

  # if sign.val is not of length 1 take the first
  sign.val <- match.arg(sign.val)
  ...
  if (missing(clinical.mat)) {
    ...
    if (!biserial.fl) {
      ...
      p = pheatmap::pheatmap(expr.mat,
                             ...
                             cellwidth = cell.size, cellheight = cell.size, fontsize = font.size,
                             fontsize_row = cell.size, fontsize_col = cell.size, silent = TRUE,
                             main = paste0("Heatmap of genes over samples (", sign.val, ")"))
      ...
      p = pheatmap::pheatmap(expr.mat,
                             ...
                             annotation = clinical.mat, breaks = breaks.hm,
                             cellwidth = cell.size, cellheight = cell.size, fontsize = font.size,
                             fontsize_row = cell.size, fontsize_col = cell.size, silent = TRUE, main = paste0("Heatmap of genes over samples (", sign.val, ")"))
  ...
  heatmap.list[["correlogram"]] = correlogram_pheatmap(t_data_heat_map, clinical.mat, scale.fl = scale.fl, signif.stars.fl = signif.stars.fl, main.correl = paste0("Gene Correlogram (", sign.val, ")"))
  ...
  heatmap.list[["correlogram.small"]] = correlogram_pheatmap(t_data_heat_map, clinical.mat, scale.fl = scale.fl, signif.stars.fl = signif.stars.fl,
                                                             main.correl = paste0("Gene Correlogram (", sign.val, ")"),  cell.size =  cell.new, font.size = font.new)
  ...
  heatmap.list[["correlogram.sample"]] = correlogram_pheatmap(as.matrix(expr.mat), clinical.mat, scale.fl = scale.fl, signif.stars.fl = signif.stars.fl,
                                                              main.correl = paste0("Sample Correlogram (", sign.val, ")"), sample.correl = TRUE, cell.size =  cell.new, font.size = font.new)

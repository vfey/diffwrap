...
prepare_volcano_of_given_property = function(data.df, property.to.plot, property.column,
  ...
  # volcano.plot <- volcano.plot + theme(axis.line.x=element_line(color="black", size = 2),
  #       axis.line.y=element_line(color="black", size = 2))
  
  volcano.plot <- volcano.plot + xlab(x.axis.name) + ylab(y.axis.name)
  volcano.plot <- volcano.plot + labs(color = name.of.legend) #guides(color=guide_legend(legend.name))
  ...
  volcano.plot <- volcano.plot + ggtitle(main, subtitle = subtitle.name) + 
    ...
    volcano.plot <- volcano.plot + geom_text(data=data.df, aes(x=floor(min(data.df$logFC)), y=-log10(property.thr)), label = line_label_text, nudge_x=-1.5, nudge_y=max(-log10(data.df[,property.to.plot]))/60, size=3, color="red")
